...
describe('buildScriptCommand', () => {
  it('uses the master install.sh for the latest version', () => {
    expect(buildScriptCommand(base)).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/TEAMO233/3x-ui/main/install.sh)',
    );
  });
  ...
  it('pins a specific version by passing the tag to master install.sh', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/TEAMO233/3x-ui/main/install.sh) v3.4.1',
    );
  });
  ...
  it('supports the rolling dev-latest build', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/TEAMO233/3x-ui/main/install.sh) dev-latest',
    );
  });
});
...
describe('buildDockerRun', () => {
  it('reflects fail2ban, port, and base path options', () => {
    ...
    expect(cmd).toContain('ghcr.io/teamo233/3x-ui:latest');
...
describe('buildDockerCompose', () => {
  it('produces valid-looking compose with the image and volumes', () => {
    ...
    expect(yaml).toContain('image: ghcr.io/teamo233/3x-ui:latest');
