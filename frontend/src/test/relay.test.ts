...
describe('relay landing outbound (manual)', () => {
  ...
  const cases: {
    input: LandingManualInput;
    schema: { safeParse: (v: unknown) => { success: boolean } };
  }[] = [
    {
      input: { ...base, protocol: 'socks', port: 1080, user: 'u', pass: 'p' },
      schema: SocksOutboundSettingsSchema,
    },
    {
      input: { ...base, protocol: 'http', port: 8080, user: 'u', pass: 'p' },
      schema: HttpOutboundSettingsSchema,
    },
    {
      input: { ...base, protocol: 'vless', port: 443, id: '8c14d6f7-2e3b-4a91-9d24-3f7a6b8c1e02' },
      schema: VlessOutboundSettingsSchema,
    },
    {
      input: { ...base, protocol: 'vmess', port: 443, id: '8c14d6f7-2e3b-4a91-9d24-3f7a6b8c1e02' },
      schema: VmessOutboundSettingsSchema,
    },
    {
      input: { ...base, protocol: 'trojan', port: 443, password: 'secret' },
      schema: TrojanOutboundSettingsSchema,
    },
    {
      input: {
        ...base,
      ...
      schema: ShadowsocksOutboundSettingsSchema,
    },
  ];
...
describe('parseLandingEndpoint', () => {
  ...
  it('parses bracketed IPv6 host:port', () => {
    expect(parseLandingEndpoint('[2001:db8::1]:1080')).toEqual({
      address: '2001:db8::1',
      port: 1080,
    });
  });
...
describe('relay rule + template splice', () => {
  ...
  it('appends outbound + prepends rule without mutating the input template', () => {
    ...
    const ob = landingOutboundFromManual(
      { protocol: 'socks', address: '203.0.113.9', port: 1080 },
      'relay-socks',
    );
  ...
  it('uniqueOutboundTag dedupes against existing tags', () => {
    ...
    expect(uniqueOutboundTag(['relay-socks', 'relay-socks-2'], 'relay-socks')).toBe(
      'relay-socks-3',
    );
  });
});
